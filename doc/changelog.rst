...
#. Added ``implicit`` integrator. Using the anaytic derivatives above, a new implicit-in-velocity integrator was added.
...
#. Public headers have been moved from ``/include`` to ``/include/mujoco/``, in line with the directory layout common in
   other open source projects. Developers are encouraged to canonicalise to the ``#include <mujoco/filename.h>`` form.
